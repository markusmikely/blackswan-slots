...
class UI:
    ...
    def display_info(self):
        ...
        balance_surf = self.font.render("Balance: $" + player_data['balance'], True, TEXT_COLOR, 'Blue')  
        x, y = 20, self.display_surface.get_size()[1] - 30
        balance_rect = balance_surf.get_rect(bottomleft = (x,y))

        bet_surf = self.bet_font.render("Wager: $" + player_data['bet_size'], True, TEXT_COLOR, 'Blue')
    ...
    def update(self):
        pygame.draw.rect(self.display_surface, 'Blue', pygame.Rect(0, 920, 1600, 80))
        self.display_info()     